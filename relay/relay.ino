 ...
 * ==================================
 * Relay與一般LED控制方式相同，皆是透過腳位電位高低來決定狀態。此範例使用的`高電平`的
 * Relay，高電平繼電器在in 腳位在高電平狀態會讓Relay呈現通電的狀態，相反的在低電平狀
 * 態下則是呈現短路狀態。
 * 
 * 如果今天使用`低電平`的Relay，`in`腳位在高電位會讓繼電器呈現短路狀態，低電位則會讓
 * Relay呈現通電的狀態。
 */
